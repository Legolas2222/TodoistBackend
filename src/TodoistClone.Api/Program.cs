...
builder.Services.AddSingleton<ITodosContext, TodosContext>(sp => new TodosContext(
    sp.GetRequiredService<MongoDBDatabaseSettings>().ConnectionString,
    sp.GetRequiredService<MongoDBDatabaseSettings>().DatabaseName,
    sp.GetRequiredService<MongoDBDatabaseSettings>().CollectionName
    ));
